...
parser.add_argument("--margin-cm", type=float, default=2.0, help="Margin around page in cm (default: 4.0)")
...
with PdfPages(out_path) as pdf:
    ...
    for page_num in range(2):
        ...
        footer_text = f"Сложность (Interest Score): {', '.join(f'{s:.1f}' for s in page_scores)}    Target = {args.target}     Time = {args.time}"
