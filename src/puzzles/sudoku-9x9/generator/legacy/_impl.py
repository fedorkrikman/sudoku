...
PUZZLE_ROOT = Path(__file__).resolve().parents[2]
SOLVER_PATH = PUZZLE_ROOT / "solver" / "legacy" / "__init__.py"
_SOLVER_MODULE_NAME = "puzzle_sudoku_9x9_solver_legacy"

_spec = importlib.util.spec_from_file_location(_SOLVER_MODULE_NAME, SOLVER_PATH)
if _spec is None or _spec.loader is None:  # pragma: no cover - defensive guard
    raise ImportError(f"Unable to load legacy solver module from '{SOLVER_PATH}'")

_solver_module = importlib.util.module_from_spec(_spec)
sys.modules[_SOLVER_MODULE_NAME] = _solver_module
_spec.loader.exec_module(_solver_module)

Grid = _solver_module.Grid
LogicSolver = _solver_module.LogicSolver
